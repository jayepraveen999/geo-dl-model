...
train_data_dir = "/home/jayendra/thesis/geo-dl-fire-detection/train_test_split_data_files/train_split/dynamic_files"
h5_dir = f"{train_data_dir}/"

fire_fraction = calculate_fire_fraction(h5_dir)
fires = [256 * 256 * x for x in fire_fraction]
train_val_fire_indices = [i for i, x in enumerate(fires) if x > 0.0]

print(f"Number of train samples with fire pixels: {len(train_val_fire_indices)}")
no_of_samples= len(train_val_fire_indices)
no_of_bands = 6
timeseries_length = 4
...
ahi_stat_p_updated = np.empty((no_of_samples, no_of_bands, 2), dtype=np.float32)


def get_subset_fires_h5_data():

    for idx, train_idx in enumerate(train_val_fire_indices):
        h5_path, preceeding_index = get_file_path(train_idx, split="train", data_dir="/home/jayendra/thesis/geo-dl-fire-detection/train_test_split_data_files/train_split/dynamic_files")
        # calculate h5_idx that is relative to the start of the h5 file
        h5_idx = train_idx - (preceeding_index + 1)
        ...
        with h5py.File(h5_path, "r") as f:
            ...
            ahi_stat_p_updated[idx, ...] = f["input_features"]["ahi_stat_p_updated"][h5_idx]
    ...
    return timestamps_str, ahi_data, cloud_mask_binary, ahi_stat_p, ahi_stat_p_updated, ahi_stat_p_c, fire_fraction, cloud_fraction, raster_window_id, labels_data


timestamps_str, ahi_data, cloud_mask_binary, ahi_stat_p, ahi_stat_p_updated, ahi_stat_p_c, fire_fraction, cloud_fraction, raster_window_id, labels_data = get_subset_fires_h5_data()


# write to hdf5
filename = f'/home/jayendra/thesis/geo-dl-fire-detection-model/data/subset_training_dynamic_data.h5'
...
with h5py.File(filename, 'w') as f:
    ...
    input_features.create_dataset('ahi_stat_p_updated', data=ahi_stat_p_updated, chunks = (1,6,2), compression="lzf")
    ...
    input_features['ahi_stat_p_updated'].attrs['description'] = "Updated AHI statistics for parent timestamp. Includes mean and standard deviation for each band"
...
print("HDF5 file created successfully for subset of training dynamic data.")